//2026/06/29  Shadowrocket 移植版
/*
@Name：WeTalk 自动化签到+视频奖励（Shadowrocket）
@Author：TG@ZenMoFiShi  移植：Claude

━━━━━━━━ Shadowrocket 配置说明 ━━━━━━━━

[Script]
# 抓包：打开 WeTalk App 触发，自动保存账号（需已登录）
http-request ^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus script-path=<脚本URL>, requires-body=false, tag=WeTalk抓包

# 定时签到：每天 08:20 和 20:20 自动执行
cron "20 8,20 * * *" script-path=<脚本URL>, tag=WeTalk签到
...
【移植说明 — Shadowrocket vs QuantumultX 差异】
1. $task.fetch()            → httpGet() 包装 $httpClient.get（回调→Promise，含重试）
2. $prefs.valueForKey()     → $persistentStore.read()
3. $prefs.setValueForKey()  → $persistentStore.write()
4. $notify(name,title,body) → $notification.post(name,title,body)
5. 顶层裸 return            → if/else 分支规避（SR 不隐式包函数）
*/
...
function MD5(string) {
  ...
  function ConvertToWordArray(str) {
    ...
    const lNW = ((((lMessageLength+8)-(lMessageLength+8)%64)/64)+1)*16;
    const lWordArray = Array(lNW-1).fill(0);
    ...
    lWordArray[lWordCount2]|=0x80<<((lByteCount%4)*8);
    lWordArray[lNW-2]=lMessageLength<<3;
    lWordArray[lNW-1]=lMessageLength>>>29;
    return lWordArray;
  }
  function WordToHex(lValue){
    let s='';
    for(let i=0;i<=3;i++){const b=(lValue>>>(i*8))&255;const t='0'+b.toString(16);s+=t.substr(t.length-2,2);}
    return s;
  }
...
/**
 * 存储读取：$persistentStore.read 替换 $prefs.valueForKey
 */
function loadStore() {
...
/**
 * 存储写入：$persistentStore.write 替换 $prefs.setValueForKey
 */
...
/**
 * HTTP GET，将 $httpClient.get 回调包装为 Promise，与原版 $task.fetch 保持相同返回格式。
 * 内置重试逻辑（最多 retry 次，仅针对网络类错误）。
 */
const RETRY_RE = /SSL|SSLSessionState|timeout|timed out|reset|connection|network|stream closed|closed|EOF/i;

function httpGet(url, headers, retry) {
  retry = (retry === undefined) ? 3 : retry;
  return new Promise((resolve, reject) => {
    $httpClient.get({ url, headers }, (error, _response, data) => {
      if (error) {
        const msg = String(error);
        if (retry > 0 && RETRY_RE.test(msg)) {
          setTimeout(() => httpGet(url, headers, retry - 1).then(resolve).catch(reject), 1200);
        } else {
          reject({ error: msg });
        }
      } else {
...
function runAccount(acc, index, total) {
  ...
  function fetchApi(path, useFakeId, retry) {
    ...
    return httpGet(buildUrl(path, acc.capture, overrideId), headers, retry);
  }
...
// ─── 入口：抓包模式 vs 定时任务模式 ─────────────────────────────────────────
// 注意：Shadowrocket 不像 QX 会隐式包裹函数，顶层不能有裸 return。
// 此处用 if/else 完整分支，彻底规避该问题。
...
if (typeof $request !== 'undefined' && $request) {
  ...
  const email = emailKeyOf(paramsRaw);
  if (!email) {
    // 未取到 email：通知并放行，不保存（用 if/else 代替原版顶层 return）
