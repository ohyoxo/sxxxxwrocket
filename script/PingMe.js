...
[Script]
# 抓包：打开 PingMe App 触发，自动保存账号
http-request ^https:\/\/api\.pingmeapp\.net\/app\/queryBalanceAndBonus script-path=<脚本URL>, requires-body=false, tag=PingMe抓包

# 定时签到：每天 08:30 和 20:30 自动执行
cron "30 8,20 * * *" script-path=<脚本URL>, tag=PingMe签到
...
1. $task.fetch()            → httpGet() 包装 $httpClient.get（回调→Promise）
2. $prefs.valueForKey()     → $persistentStore.read()
3. $prefs.setValueForKey()  → $persistentStore.write()
4. $notify(name,title,body) → $notification.post(title,sub,body)
    ...
    return new Promise((resolve, reject) => {
      $httpClient.get({ url, headers }, (error, _response, data) => {
        if (error) {
          ...
          if (n < 3 && RETRY_RE.test(msg)) {
            // 等待 1500ms 后重试
            setTimeout(() => attempt(n + 1).then(resolve).catch(reject), 1500);
          } else {
            reject({ error: msg });
          }
        } else {
...
if (typeof $request !== 'undefined' && $request) {
  ...
  store.accounts[fp] = {
    id: fp, alias, email, uaSeed, baseUA,
